...
function clearAuthCookies() {
  return [
    cookieSerialize("session", "", {
      maxAge: 0,
      path: "/",
      secure: true,
      sameSite: "None"
    }),
    cookieSerialize("refresh", "", {
      maxAge: 0,
      path: "/",
      secure: true,
      sameSite: "None"
...
async function authRegister(body, env) {
  ...
  if (password.length < 8) return json({
    ok: false,
    error: "weak password"
  }, 400);
  ...
  const {
    cookies: setCookies
  } = await issueSessionCookies(user, env);
...
async function issueSessionCookies(user, env) {
  ...
  const session = await jwtSign({
    ...
    email: user.email,
    roles
  }, env.AUTH_SECRET, accessTtl);
  const jti = newId(16);
  const refresh = await jwtSign({
    sub: user.id,
    jti
  }, env.AUTH_SECRET, refreshTtl);
...
async function googleStart(url, env) {
  ...
  const stateObj = {
    r: redirect,
    n: b64urlStr(String.fromCharCode(...crypto.getRandomValues(new Uint8Array(8))))
  };
...
async function googleCallback(url, env) {
  ...
  if (!env.DB || !env.DB.prepare) {
    return json({
      ok: false,
      error: "D1 binding 'DB' mancante sul Worker"
    }, 500);
  }
  ...
  if (!tokenRes.ok) return json({
    ok: false,
    error: tok.error_description || "oauth error"
  }, 400);
  ...
  if (!email || !sub) return json({
    ok: false,
    error: "invalid google profile"
  }, 400);
